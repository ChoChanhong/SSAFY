...
contract Confirmation is ERC721Connector {
    ...
    mapping (uint => uint) _countOfId;  // 처방전 토큰아이디 별 조제한 횟수
        ...
        uint _dispensingcount)
    {
        ...
        for (uint i = 0; i < cfms.length; i++){
            if(cfms[i].id == _no && cfms[i].dispensingCount == _dispensingcount){
                check = true;
            }
        }
    ...
    //  cfmExists (_confirmation.No, _confirmation.dispensingCount)
    function mint(confirmation memory _confirmation)
        public cfmExists (_confirmation.id, _confirmation.dispensingCount){
